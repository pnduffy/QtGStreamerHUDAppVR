...
Rectangle {
    ...
    property real imageWidth : compassImage.width
    ...
    border.width: (5 * zoom)
    transform: Translate{
        y: parent.height - (compassImage.height/2) - (2*root.mm)
    }
    ...
    Image {
        ...
        sourceSize.height: 3*zoom
        sourceSize.width: 3*zoom
        smooth: true
    }

    Rectangle{
        anchors.verticalCenter: parent.verticalCenter
        anchors.verticalCenterOffset: compassImage.height/2 - (4 * zoom)

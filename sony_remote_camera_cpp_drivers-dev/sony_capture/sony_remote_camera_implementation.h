...
namespace src {
	...
	public:
		Sony_Remote_Camera_Implementation(std::string my_own_ip_address, std::string camera_service_url, std::string liveview_url);
		~Sony_Remote_Camera_Implementation();
	...
	private:
		...
		bool got_it;
		int current_json_id;
		std::map<Camera_State::Shoot_Mode, std::string> shoot_modes_availables;

...
Sony_Remote_Camera_Interface* src::GetSonyRemoteCamera(const char * my_own_ip, const char *camera_service_url, const char *liveview_url) {
	try {
		return new Sony_Remote_Camera_Implementation(my_own_ip, camera_service_url, liveview_url);
	}
...
Sony_Remote_Camera_Implementation::Sony_Remote_Camera_Implementation(string my_own_ip_address, string camera_service_url, string liveview_url) 
	:data(NULL),
	SSDPClient(io_service,my_own_ip_address, camera_service_url),
	...
	if (camera_service_url.length() > 0 || liveview_url.length() > 0)
	{
		// Skip discovery if url specified
		this->liveview_url = liveview_url;
		this->camera_service_url = camera_service_url;
	}
	else
	{
		io_service.run();
	...
	shoot_modes_availables.insert(std::pair<Camera_State::Shoot_Mode, std::string>(Camera_State::STILL, "\"still\""));
	shoot_modes_availables.insert(std::pair<Camera_State::Shoot_Mode, std::string>(Camera_State::MOVIE, "\"movie\""));
	shoot_modes_availables.insert(std::pair<Camera_State::Shoot_Mode, std::string>(Camera_State::AUDIO, "\"audio\""));
	shoot_modes_availables.insert(std::pair<Camera_State::Shoot_Mode, std::string>(Camera_State::INTERVALSTILL, "\"intervalstill\""));
}
...
void Sony_Remote_Camera_Implementation::Retrieve_Decription_File() {
	const string description_url(SSDPClient.Get_Description_Url());
...
Sony_Capture_Error Sony_Remote_Camera_Implementation::Launch_Liveview()
{
	...
	string json_request(Build_JSON_Command("startLiveview", params));// (("{\"method\": \"startLiveview\",\"params\" : [],\"id\" : 1,\"version\" : \"1.0\"}\r\n");
	...
	liveview_thread = thread(boost::bind(&Sony_Remote_Camera_Implementation::Read_Liveview_Continuously, this));
	return SC_NO_ERROR;
}
...
void Sony_Remote_Camera_Implementation::Handle_Read_Common_Header(const boost::system::error_code& err, size_t payload_bytes_already_read, size_t image_bytes_already_read) {
	vector<uint8_t> lv_on;
	lv_on.push_back(0xff);
	lv_on.push_back(0x01);
...
void Sony_Remote_Camera_Implementation::Handle_Read_Image(const boost::system::error_code& err) {
	...
	ts = 0;
	fn = 0;
	vector<uint8_t> start_marker;
	start_marker.push_back(0xff);
	start_marker.push_back(0xd8);
	vector<uint8_t> end_marker;
	end_marker.push_back(0xff);
	end_marker.push_back(0xd9);
...
Sony_Remote_Camera_Implementation::Set_Shoot_Mode(Camera_State::Shoot_Mode mode) {
	std:vector<string> params;
	params.push_back(shoot_modes_availables.at(mode));
	string command = Build_JSON_Command("setShootMode", params);
	...
	int count = 0;
	while (!check_shoot_mode() && count<10){
		count++;
...
void Sony_Remote_Camera_Implementation::Check_Events(){
	...
	while (true) {
		...
		try {
			Send_Options_Command(Build_JSON_Command("getEvent", params), true);
...
Sony_Capture_Error Sony_Remote_Camera_Implementation::Set_Recording(bool start){
	...
	std::vector<string> params;
	Send_Options_Command(Build_JSON_Command(s, params));
	return SC_NO_ERROR;
}

Sony_Capture_Error Sony_Remote_Camera_Implementation::Zoom(bool in, bool start)
{
	string s = "actZoom";
	std::vector<string> params;
	if (in) params.push_back("\"in\"");
	else params.push_back("\"out\"");

	if (start) params.push_back("\"start\"");
	else params.push_back("\"stop\"");

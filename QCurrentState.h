...
public:
    ...
    void setDistUnit(QString unit) { m_distUnit = unit.isEmpty() ? "m" : unit; emit distUnitChanged(unit); }
    void setSpeedUnit(QString unit) { m_speedUnit = unit.isEmpty() ? "m/s" : unit; emit speedUnitChanged(unit); }
    ...
    double GetDistanceBetweenPoints(QPointF curPos, QPointF lastPos);
...
private slots:
    ...
    void onValueChanged(const int uasid, const QString& name, const QString& unit, const QVariant &value,const quint64 msecs);
    void onArmed();
    void onDisarmed();
    void onRemoteControlChannelScaledChanged(int channelId, float normalized);
    void updateTimeInAir();
    void onGlobalPositionChanged(UASInterface*, double lat, double lon, double alt, quint64 usec);
